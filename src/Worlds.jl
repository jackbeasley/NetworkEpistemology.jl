...
# TODO: StaticArrays
struct BinomialActionFacts{PT <: Real}
...
function observe_fact(baf::BinomialActionFacts, factID::Int)::BinomialActionObservation
    return BinomialActionObservation(
        factID,
        rand(baf.actionDistributions[actionID]),
        baf.trialsPerStep
    )
end
...
TotalObservations(numActions::Int) = TotalObservations([0 for _ in 1:numActions], [0 for _ in 1:numActions])

# TODO: Functional vs mutable style
function observe_results(baf::BinomialActionObservation, observations_ref::TotalObservations)::TotalObservations
...
function step(prev::ZollmanModelState)::ZollmanModelState
    per_individual_observations = [TotalObservations(num_facts(pref.facts)) for _ in 1:length(prev.individuals)]
    for indiv in prev.individuals
        # TODO: Action -> Fact to observe
        actionID = select_action(indiv)
        observation = observe_facts(prev.facts, actionID)
        ...
        for neighborID in inneighbors(prev.structure)
            observe_results(observation, per_individual_observations[neighborID])
        end
    end

    new_individuals = [update_with_observations(indiv, observation) for (indiv, observation) in zip(w.individuals, per_individual_observations)]
